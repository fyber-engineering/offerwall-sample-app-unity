...
extern "C" {
    ...
    void fyb_ofw_start(const char *app_id, const char *security_token) {
        ...
        dispatch_once(&onceToken, ^{
            ...
            OFWVirtualCurrencyStartOptions *vcsOptions = [OFWVirtualCurrencyStartOptions optionsWithSecurityToken:securityToken delegate:gFairBidDelegate];

            [OfferWall startWithAppId:appId delegate:gFairBidDelegate virtualCurrencyStartOptions:vcsOptions completion:^(OFWError * _Nullable) {
    ...
    void fyb_ofw_request_virtual_currency(BOOL toast_on_reward, const char * currency_id) {
        ...
        OFWCurrencyRequestOptions *options = [OFWCurrencyRequestOptions optionsWithCurrencyId:currencyId toastOnReward:toast_on_reward];
